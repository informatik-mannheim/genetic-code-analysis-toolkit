...
import bio.gcat.geneticcode.core._
...
trait GeneticCodeTable {
   ...
   * @return
   */
  def translate(codon: Codon): CodonMapping

  def reverseTranslate(m: CodonMapping) = {
    codons.filter {
      codon =>
  ...
  def reverseList = {
    ...
    // them in a tuple:
    aminoAcids.map(aa => AminoAcidMapping(aa.toString)).map(a => (a, reverseTranslate(a)))
  }
...
abstract class AbstractBioJavaTable extends Table with GeneticCodeTable {
  ...
  def getCodons(nucelotides: CompoundSet[NucleotideCompound],
    ...
    val codons = for (b1 <- rnaCompounds;
      ...
      val aac = aa match {
        case UnknownCodonMapping => aminoAcids.getCompoundForString("*")
        case am: AminoAcidMapping => am.aminoAcid.get
      }
      val isStop = aa match {
        case geneticcode.core.UnknownCodonMapping => true
        case _ => false
      }
...
class StandardGeneticCodeTable extends AbstractBioJavaTable {
  ...
  def translate(codon: Codon) = {
    ...
    // TODO consider exceptions
      geneticcode.core.AminoAcidMapping(aa.toString)
  }
}